...
export const upsertSupplier = async (req: Request, res: Response): Promise<void> => {
    ...
    try {
        const result = await prisma.$transaction(async (prisma) => {
            ...
            const checkExistingPhone = await prisma.suppliers.findFirst({
                where: { phone, deletedAt: null, id: { not: Number(supplierId) } }
            });
            if (checkExistingPhone) {
                res.status(400).json({ message: "Supplier with the same phone already exists!" });
                return;
            }

            const checkExistingEmail = await prisma.suppliers.findFirst({
                where: { email, deletedAt: null, id: { not: Number(supplierId) } }
            });
            if (checkExistingEmail) {
                res.status(400).json({ message: "Supplier with the same email already exists!" });
                return;
            }
