...
export const upsertService = async (req: Request, res: Response): Promise<void> => {
    ...
    try {
        const restult = await prisma.$transaction(async (prisma) => {
            ...
            const checkExistingCode = await prisma.services.findFirst({
                where: { serviceCode, id: { not: Number(serviceId) } }
            });
            ...
            const checkExistingName = await prisma.services.findFirst({
                where: { name, id: { not: Number(serviceId) } }
            });
